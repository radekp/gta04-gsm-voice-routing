...
/* Define this to use Andre Adrian's echo cancellation
   http://andreadrian.de/echo_cancel/draft-aec-03.txt */
#define USE_ANDRES_AEC
...
#define u16 unsigned short
    ...
    struct route_stream
    {
        ...
        const char *pcm_name;   // in: "default" or "hw:1,0"
...
/* Dump error on stderr with stream and error description, and return given
   return_code */
    int err(const char *msg, int snd_err, struct route_stream *s,
            int return_code)
    {
    ...
    int open_route_stream(struct route_stream *s)
    {
        ...
        if (rc < 0) {
            return err("snd_pcm_hw_params_any failed", rc, s,
                       ERR_HW_PARAMS_ANY);
        }
        ...
        if (s->period_buffer == 0) {
            return err("period_buffer alloc failed", 0, s,
                       ERR_BUFFER_ALLOC_FAILED);
        }
        ...
        if (s->start_threshold > 0 || s->stop_threshold > 0) {
            ...
            if (s->start_threshold > 0) {
                rc = snd_pcm_sw_params_set_start_threshold(s->handle,
                                                           ...
                                                           s->start_threshold);
                if (rc < 0) {
                    return err("snd_pcm_sw_params_set_start_threshold failed",
                               rc, s, ERR_SW_PARAMS_SET_START_THRESHOLD);
                }
            }
            ...
            if (s->stop_threshold > 0) {
                ...
                if (rc < 0) {
                    return err("snd_pcm_sw_params_set_start_threshold failed",
                               rc, s, ERR_SW_PARAMS_SET_STOP_THRESHOLD);
                }
            }
...
*/
    void reduce_echo(char *p0, char *p1, snd_pcm_uframes_t period_size)
    {
        ...
        for (i = 0; i < period_size; i++) {
            ...
            if ((i % 32) == 31) {
                ...
                if (sum0 > 3000 && sum1 < 16000) {
                    memset(v1 - 31, 0, 32); // other side is talking, mute mic
                    listening++;
                }
                if (sum1 > 16000) {
                    talking++;
        ...
        if (listening == 0 && talking == 0) {
            set_aux_leds(0, 0);
        } else if (listening > talking) {
            set_aux_leds(0, 1);
        } else {
            set_aux_leds(1, 0);
        }
    ...
    int main()
    {
        ...
        p0.id = "p0";
        p0.pcm_name = "default";
        p0.stream = SND_PCM_STREAM_PLAYBACK;
        p0.start_threshold = 1024;
        p0.stop_threshold = 1024;
        p0.buffer_size = 1024;
        p0.period_size = 256;
        p0.handle = 0;
        p0.period_buffer = 0;
        ...
        r0.id = "r0";
        r0.pcm_name = "default";
        r0.stream = SND_PCM_STREAM_CAPTURE;
        r0.start_threshold = 0;
        r0.stop_threshold = 0;
        r0.buffer_size = 1024;
        r0.period_size = 256;
        r0.handle = 0;
        ...
        p1.id = "p1";
        p1.pcm_name = "hw:1,0";
        p1.stream = SND_PCM_STREAM_PLAYBACK;
        p1.start_threshold = 1024;
        p1.stop_threshold = 1024;
        p1.buffer_size = 1024;
        p1.period_size = 256;
        p1.handle = 0;
        p1.period_buffer = 0;
        ...
        r1.id = "r1";
        r1.pcm_name = "hw:1,0";
        r1.stream = SND_PCM_STREAM_CAPTURE;
        r1.start_threshold = 0;
        r1.stop_threshold = 0;
        r1.buffer_size = 1024;
        r1.period_size = 256;
        r1.handle = 0;
...
#ifdef USE_ANDRES_AEC
            int i;
            s16 *vp0 = (s16*)(p0.period_buffer);
            s16 *vr0 = (s16*)(r0.period_buffer);
            s16 *vp1 = (s16*)(p1.period_buffer);
            for (i = 0; i < p0.period_size; i++) {
                *vp1 = aec.doAEC(*vr0, *vp0);
                vp0++;
                vr0++;
                vp1++;
            }
